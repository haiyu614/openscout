...
| **M6** | ✅ 已完成（待推送） | Core：PRWorkflowEngine 扩展（`review/approved:revise→revising`、`revising:submit-for-review→review`、`revising:fail→failed`、版本递增 `nextVersion`/`canRevise`）；`ContribOrchestrator.revise`（多轮修改：重新打开→Agent→新版本审阅包→回到 review）+ `listWorkItems`；DedupEngine 完善（`recordPublication`/`publishedPRNumbersFor` 跨轮/跨任务已发布 PR 跟踪，规则 4 增强；墓碑/跨任务/意图去重强化）；PublishEngine 发布成功写回去重；adapter-dsh：`workitem-tools.ts`（`openscout_revise`/`openscout_list_workitems`/`openscout_reset_workitem`）；复用 M3/M4 引擎 | `tsc --build` 通过；`vitest` 140/140 通过（M6 +15：状态机 5 / revise 5 / 去重 5）；Core 零 DSH 依赖（见 `docs/m6-evaluation.md`） |
